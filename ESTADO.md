...
## Problemas conocidos
- **veredicto:landing — NO LISTA, pospuesto a propósito**: `docs/revisiones/landing-veredicto.md` (7ª pasada) dice "NO LISTA" porque usabilidad quedó en 35/40 (gate: ≥36) — craft (18/20) y copy (19/20) SÍ pasan. El único defecto restante es que el `<header>` de marca (logo + "Entrar") no obtiene el rol ARIA "banner" por vivir dentro de `<section>`/`<main>` (regla de exclusión de landmarks anidados de HTML/ARIA) — invisible para un usuario normal, solo afecta cómo un lector de pantalla anuncia el encabezado. Corregirlo de raíz exige que `Hero.tsx` deje de renderizar su propio `<header>` y que `page.tsx` lo renderice como hermano de `<main>` — una reestructuración del kit, no una tematización de tokens. Se decidió posponerlo a la pasada de accesibilidad de `06-TESTING.md` en Sesión 7 (donde se audita accesibilidad de TODA la app junta) en vez de seguir iterando 1 punto en una sola pantalla. Se le presentó esta decisión al usuario explícitamente antes de continuar.
