...
## Problemas conocidos
- **veredicto:landing — NO LISTA, pospuesto a propósito (actualizado tras la 9ª pasada en curso)**: base: 35/40 usabilidad (gate ≥36) — craft (18/20) y copy (19/20) YA PASAN. Defecto de fondo sin resolver: el `<header>` de marca (logo + "Entrar") no obtiene el rol ARIA "banner" por vivir dentro de `<section>`/`<main>` (regla de exclusión de landmarks anidados de HTML/ARIA) — invisible para un usuario normal, solo afecta cómo un lector de pantalla anuncia el encabezado. Corregirlo de raíz exige que `Hero.tsx` deje de renderizar su propio `<header>` y que `page.tsx` lo renderice como hermano de `<main>` — una reestructuración del kit, no una tematización de tokens. Pospuesto a la pasada de accesibilidad de `06-TESTING.md` en Sesión 7, decisión ya presentada al usuario.
  - Tras la 7ª pasada se agregó la fila de 3 beneficios con ícono en el Hero (pedida por el usuario) → la 8ª pasada bajó a 34/40 por un defecto NUEVO: esos chips no reusaban `IconChip` del kit (36px sin borde vs 44px con borde de Problema/Solución) → **corregido**: ahora usan `IconChip` directamente, mismo componente que el resto de la página. 9ª pasada relanzada para confirmar; su resultado se registra aquí en cuanto llegue.
