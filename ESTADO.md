...
## Problemas conocidos
- **veredicto:landing — NO LISTA, pospuesto a propósito (actualizado tras la 8ª pasada)**: `docs/revisiones/landing-veredicto.md` venía en 35/40 usabilidad (gate ≥36) — craft (18/20) y copy (19/20) YA PASAN. El único defecto de fondo es que el `<header>` de marca (logo + "Entrar") no obtiene el rol ARIA "banner" por vivir dentro de `<section>`/`<main>` (regla de exclusión de landmarks anidados de HTML/ARIA) — invisible para un usuario normal, solo afecta cómo un lector de pantalla anuncia el encabezado. Corregirlo de raíz exige que `Hero.tsx` deje de renderizar su propio `<header>` y que `page.tsx` lo renderice como hermano de `<main>` — una reestructuración del kit, no una tematización de tokens. Se decidió posponerlo a la pasada de accesibilidad de `06-TESTING.md` en Sesión 7 (donde se audita accesibilidad de TODA la app junta), decisión ya presentada al usuario. Después de esa 7ª pasada se agregó la fila de 3 beneficios con ícono en el Hero (pedida por el usuario, mejora de escaneabilidad) — se relanzó el revisor-visual (8ª pasada) para confirmar que ese cambio no rompió nada y para la auditoría de escaneabilidad; su resultado se registra aquí en cuanto llegue.
